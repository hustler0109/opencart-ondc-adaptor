...
const { getCallbackUri } = require("../utils/registryLookup");
...
const statusHandler = async (req, res) => {
  ...
  // ACK early
  res.status(200).json(sendAck());

  // Start async post-ACK work
  setImmediate(async () => {
    try {
      ...
      const onStatusPayload = {
        ...
        message: {
          order: {
            id: orderId,
            state: mappedState,
            fulfillments: [], // Optional: Add tracking data here if needed
          }
        }
      };
